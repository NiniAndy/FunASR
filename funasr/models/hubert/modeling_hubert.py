...
class BatchMatrixMultiplication(nn.Module):
    """
    used for IB analysis, in regular case, use torch.bmm
    It is convenient to mount hooks to the location where attention and value are multiplied
    """
...
class HubertAttention(nn.Module):
    ...
    def __init__(
        ...
        self.att_softmax = nn.Softmax(dim=-1)   # only for IB analysis
        self.att_v_bmm = BatchMatrixMultiplication()    # only for IB analysis, in regular case, use torch.bmm
    ...
    def forward(
        ...
        attn_weights = self.att_softmax(attn_weights)
        ...
        attn_output = self.att_v_bmm(attn_probs, value_states)

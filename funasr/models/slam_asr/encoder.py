...
@tables.register("encoder_classes", "WhisperWrappedEncoder")
class WhisperWrappedEncoder:

    @classmethod
    def load(cls, **encoder_conf: Dict):
        ...
        whisper_decode = encoder_conf.get("whisper_decode", False)
        if whisper_decode:
            import whisper
            encoder_path = encoder_conf["encoder_path"]
            whisper_model = whisper.load_model(name=encoder_path, device='cpu')
        ...
        encoder_path_hf = encoder_conf.get("encoder_path_hf", None)
        if encoder_path_hf is not None:
            ...
            encoder = WhisperModel.from_pretrained(encoder_path_hf).encoder
        else:
            import whisper
            encoder_path = encoder_conf["encoder_path"]
            encoder = whisper.load_model(name=encoder_path, device='cpu').encoder
...
#             w2v2_config_path=model_config.get('encoder_conf_path', "facebook/wav2vec2-conformer-rope-large-960h-ft")
#         )

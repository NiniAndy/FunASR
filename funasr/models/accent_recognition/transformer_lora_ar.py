...
@tables.register("model_classes", "TransformerLoRAAr")
class TransformerLoRAAr(nn.Module):
    """
    1.加载一个训练好的base模型
    2.利用该base模型提供原始的encoder_out
    3.用lora训练加载的base模型
    4.用lora的encoder_out和base的encoder_out做Ar的训练
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        asd = ASD(asd_input_size, acc_num, num_spaces=num_space, **lasas_conf)
        ...
        anchor_kwargs["tokenizer_conf"]["token_list"] = "/ssd/zhuang/code/FunASR/examples/kespeech/DATA/data/zh_token_list/char/tokens.txt"
        ...
        # anchor_kwargs["frontend"] = None
        # anchor_kwargs["frontend_conf"] = None
        anchor_kwargs["frontend_conf"]["cmvn_file"] = "/ssd/zhuang/code/FunASR/examples/kespeech/DATA/data2/WD/train/am.mvn"
        # anchor_kwargs["ctc_conf"] = None
        anchor_kwargs["init_param"] = os.path.join(anchor_base_path, "model.pt.avg10")
        if len(encoder.interctc_layer_idx) != 0:
        ...
        # 只保留decoder embed其他的decoder部分删掉
        anchor_tokenizer = anchor_model.decoder.embed
        delattr(anchor_model, 'decoder')
        self.anchor_model = anchor_model
        self.anchor_tokenizer = anchor_tokenizer
        for param in self.anchor_model.parameters():
            param.requires_grad = False
        for param in self.anchor_tokenizer.parameters():
            param.requires_grad = False
        anchor_ctc = CTC(odim=anchor_model.vocab_size, encoder_output_size=anchor_kwargs["encoder_conf"]["output_size"], **anchor_kwargs["ctc_conf"])
        # 加载ctc参数
    ...
    def forward(
        ...
        **kwargs,
        ...
        loss_att, acc_att, cer_att, wer_att = self._calc_att_loss(
            decoder_input, decoder_input_lens, text, text_lengths, langauge_emb
        )

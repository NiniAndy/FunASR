...
def attention_beam_search(
    ...
    for i in range(prefix_len, maxlen + 1):
        ...
        # logp: (B*N, vocab)
        logp, cache = model.text_decoder.forward_step_by_step(encoder_out, encoder_mask, hyps, hyps_mask, cache)
...
def mix_beam_search(
        ...
        text_encoder_mask: torch.Tensor,
        beam_size: int = 10,
        length_penalty: float = 0.0,
        **kwargs,
) -> List[DecodeResult]:
    device = text_encoder_out.device
    batch_size = text_encoder_out.shape[0]
    # Let's assume B = batch_size and N = beam_size
    # 1. Encoder
    running_size = batch_size * beam_size

    text_maxlen = text_encoder_out.size(1)
    text_encoder_dim = text_encoder_out.size(2)
    text_encoder_out = text_encoder_out.unsqueeze(1).repeat(1, beam_size, 1, 1).view(running_size, text_maxlen, text_encoder_dim)  # (B*N, text_maxlen, text_encoder_dim)
    text_encoder_mask = text_encoder_mask.unsqueeze(1).repeat(1, beam_size, 1, 1).view(running_size, 1, text_maxlen)  # (B*N, 1, max_len)

    audio_maxlen = audio_encoder_out.size(1)
    audio_encoder_dim = audio_encoder_out.size(2)
    audio_encoder_out = audio_encoder_out.unsqueeze(1).repeat(1, beam_size, 1, 1).view(running_size, audio_maxlen, audio_encoder_dim)  # (B*N, audio_maxlen, audio_encoder_dim)
    audio_encoder_mask = audio_encoder_mask.unsqueeze(1).repeat(1, beam_size, 1, 1).view(running_size, 1, audio_maxlen)  # (B*N, 1, max_len)
    ...
    scores = scores.to(device).repeat([batch_size]).unsqueeze(1).to(device)  # (B*N, 1)
    end_flag = torch.zeros_like(scores, dtype=torch.bool, device=device)
    text_cache: Optional[List[torch.Tensor]] = None
    audio_cache: Optional[List[torch.Tensor]] = None

    # 2. Decoder forward step by step
    for i in range(prefix_len, text_maxlen + 1):
        # Stop if all batch and all beam produce eos
        if end_flag.sum() == running_size:
            break
        # 2.1 Forward decoder step
        hyps_mask = subsequent_mask(i).unsqueeze(0).repeat(running_size, 1, 1).to(device)  # (B*N, i, i)
        # text_logp: (B*N, vocab)
        text_logp, text_cache, intermediate_outs = model.text_decoder.forward_step_by_step_and_layer_by_layer(
            text_encoder_out,
            text_encoder_mask,
            hyps,
            hyps_mask,
        ...
        audio_logp, audio_cache, _ = model.decoder.forward_step_by_step_and_layer_by_layer(
            audio_encoder_out,
            audio_encoder_mask,
            hyps,
            hyps_mask,
        ...
        # 2.2 First beam prune: select topk best prob at current time
        top_k_logp, top_k_index = audio_logp.topk(beam_size)  # (B*N, N)
        top_k_logp = mask_finished_scores(top_k_logp, end_flag)
        top_k_index = mask_finished_preds(top_k_index, end_flag, model.eos)
        ...
        scores = scores.view(batch_size, beam_size * beam_size)  # (B, N*N)
        scores, offset_k_index = scores.topk(k=beam_size)  # (B, N)
        # Update text_cache to be consistent with new topk scores / hyps
        cache_index = (offset_k_index // beam_size).view(-1)  # (B*N)
        base_cache_index = (torch.arange(batch_size, device=device).view(-1, 1).repeat([1, beam_size]) * beam_size).view(-1)  # (B*N)
        cache_index = base_cache_index + cache_index
        audio_cache = [torch.index_select(c, dim=0, index=cache_index) for c in audio_cache]
        text_cache = [torch.index_select(c, dim=0, index=cache_index) for c in text_cache]
        scores = scores.view(-1, 1)  # (B*N, 1)

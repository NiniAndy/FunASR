...
from funasr.models.transformer.positionwise_feed_forward import PositionwiseFeedForward,  ProjectFeedForward
from funasr.models.transformer.utils.repeat import repeat
from funasr.models.transformer.scorers.scorer_interface import BatchScorerInterface
from funasr.models.text_transformer.attention import MultiHeadedAttentionDetailVersion
...
class BaseTransformerDecoder(nn.Module, BatchScorerInterface):
    ...
    def batch_score(self, ys: torch.Tensor, states: List[Any], xs: torch.Tensor, **kwargs) -> Tuple[torch.Tensor, List[Any]]:
...
@tables.register("decoder_classes", "TextTransformerDecoder")
class TextTransformerDecoder(BaseTransformerDecoder):
    def __init__(
        self,
        vocab_size: int,
        encoder_output_size: int,
        ...
        if combine_method == "concat":
            combine_size = encoder_output_size *2
        elif combine_method == "add":
            combine_size = encoder_output_size
        else:
            raise ValueError(f"combine_method should be 'concat' or 'add': {combine_method}")

        attention_dim = encoder_output_size

        self.projs = repeat(
            num_blocks,
            lambda lnum: ProjectFeedForward(idim=combine_size, odim=attention_dim, hidden_units=linear_units, dropout_rate=dropout_rate)
        )

        self.final_proj = ProjectFeedForward(idim=combine_size, odim=attention_dim, hidden_units=linear_units, dropout_rate=dropout_rate)
        ...
        #         MultiHeadedAttentionDetailVersion(
        #             attention_heads,
        #             attention_dim,
        #             src_attention_dropout_rate,
        #             q_in=attention_dim,
        #             k_in=encoder_output_size,
        #             v_in=encoder_output_size),
        #         PositionwiseFeedForward(idim=attention_dim, hidden_units=linear_units, dropout_rate=dropout_rate),
        #         dropout_rate,
        #         normalize_before,
        #         concat_after,
        #     ),
    ...
    def forward(
        ...
        ys_in_pad: torch.Tensor,
        ys_in_lens: torch.Tensor,
        text_decoder_output_list: list = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Forward decoder.
        ...
        text_emb = text_decoder_output_list[0][1]
        text_decoder_output_list = text_decoder_output_list[1:]
        if self.combine_method == "concat":
            x = torch.cat((x, text_emb), dim=-1)
        elif self.combine_method == "add":
            x = x + text_emb

        for layer_idx in range(len(self.decoders)):
            decoder_layer = self.decoders[layer_idx]
            proj = self.projs[layer_idx]

            x = proj(x)
            x, tgt_mask, memory, memory_mask = decoder_layer(x, tgt_mask, memory, memory_mask)

            text_emb = text_decoder_output_list[layer_idx][1]
            if self.combine_method == "concat":
                x = torch.cat((x, text_emb), dim=-1)
            elif self.combine_method == "add":
                x = x + text_emb
    ...
    def forward_step_by_step_and_layer_by_layer(
        ...
        tgt: torch.Tensor,
        tgt_mask: torch.Tensor,
        text_decoder_output_list: List[List[Tuple[int, torch.Tensor]]] = None,
        cache: List[torch.Tensor] = None,
    ):
        """Forward one step.
        ...
        for layer_idx in range(len(self.decoders)):
            ...
            if cache is None:
                ...
                c = cache[layer_idx]
            x = proj(x)
            x, tgt_mask, memory, memory_mask = decoder_layer(x, tgt_mask, memory, memory_mask, cache=c)

...
class BeamSearch(torch.nn.Module):
    ...
    def init_hyp(self, x: torch.Tensor, **kwargs) -> List[Hypothesis]:
        ...
        """
        init_sos = kwargs.get("init_sos", self.sos)
        init_states = dict()
        init_scores = dict()
        ...
        return [
            Hypothesis(
                ...
                scores=init_scores,
                states=init_states,
                yseq=torch.tensor([init_sos], device=x.device),
            )
        ]
    ...
    def score_full(
        ...
        for k, d in self.full_scorers.items():
            scores[k], states[k] = d.score(hyp.yseq, hyp.states[k], x, **kwargs)
    ...
    def search(self, running_hyps: List[Hypothesis], x: torch.Tensor, **kwargs) -> List[Hypothesis]:
        ...
        for hyp in running_hyps:
            ...
            scores, states = self.score_full(hyp, x, **kwargs)
    ...
    def forward(
        ...
        for i in range(maxlen):
            ...
            best = self.search(running_hyps, x, **kwargs)

...
@tables.register("model_classes", "TextTransformer2")
class TextTransformer2(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        pny, pny_lengths = self.ctc_out(encoder_out, encoder_out_lens, text)
    ...
    def han_sampler(self, predict, label, acc_threshold=0.6):
        # Calculate the real length of the label (ignoring -1 padding)
        real_length = (label != -1).sum().item()

        # Trim or pad predict to match the real length of the label
        if len(predict) < real_length:
            predict = predict + [self.blank_id] * (real_length - len(predict))
        elif len(predict) > real_length:
            predict = predict[:real_length]

        # Trim the label to exclude padding for accuracy computation
        trimmed_label = label[:real_length].clone().cpu()
        predict = torch.tensor(predict)

        correct_predictions = (predict == trimmed_label).sum().item()
        acc = correct_predictions / real_length if real_length > 0 else 0.0

        if acc < acc_threshold:
            # Identify correct and incorrect positions
            correct_mask = (predict == trimmed_label)  # True for correct predictions
            incorrect_mask = ~correct_mask  # True for incorrect predictions

            # Count current correct and incorrect predictions
            current_correct = correct_mask.sum().item()
            total_length = len(trimmed_label)
            target_correct = int(total_length * acc_threshold)  # Target number of correct predictions

            # If already enough correct predictions, do nothing
            if current_correct >= target_correct:
                return predict

            # Calculate how many additional correct predictions are needed
            additional_correct_needed = target_correct - current_correct

            # Get indices of currently incorrect predictions
            incorrect_indices = torch.nonzero(incorrect_mask).squeeze().tolist()

            # Randomly select indices from incorrect positions to replace
            selected_indices = random.sample(incorrect_indices, additional_correct_needed)

            # Update predict: replace selected incorrect positions with correct labels
            new_predict = predict.clone()
            for idx in selected_indices:
                new_predict[idx] = trimmed_label[idx]

            # Update predict with new adjustments
            predict = new_predict
        return predict

    def ctc_out(self, encoder_out, encoder_lens, text, stage="train"):
        with torch.no_grad():
            ...
            for i in range(len(results)):
                ...
                if stage == "train":
                    label = text[i]
                    han_id = self.han_sampler(han_id, label).tolist()
                else:
                    han_id = han_id
    ...
    def inference(
        ...
        **kwargs,
        ...
        pny, pny_lengths = self.ctc_out(encoder_out, encoder_out_lens, text=None, stage="infer")
        pny = pny.masked_fill(pny == -1, self.eos)
        if len(pny_lengths.size()) > 1:
            pny_lengths = pny_lengths[:, 0]

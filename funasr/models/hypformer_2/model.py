    ...
    def inference(
        ...
        **kwargs,
        ...
        for i in range(b):
            ...
            # attn_res_token_int = list(filter(lambda x: x != self.eos and x != self.sos and x != self.blank_id, paraformer_greedy_result_token_int))

            '''paraformer beam search'''
            # beam_size = kwargs.get("beam_size", 10)
            ...
            # beam_size = kwargs.get("beam_size", 10)
            # paraformer_beam_result = paraformer_beam_search(decoder_out, ys_pad_lens, beam_size=beam_size, eos=self.eos)
            # attn_res_result = nar2_rescore(self, prefix_results=paraformer_beam_result, encoder_outs=encoder_out,
            #                                       encoder_lens=torch.tensor([encoder_out.size(1)]), prefix_weight=attnre_weight, reverse_weight=0.0)
            # attn_res_token_int = attn_res_result[0].tokens
            # attn_res_token_int = list(filter(lambda x: x != self.eos and x != self.sos and x != self.blank_id, attn_res_token_int))
            ...
            # attnre_weight=kwargs.get("decoding_attnre_weight", 0.5)
            # paraformer_greedy_result = paraformer_greedy_search(decoder_out, ys_pad_lens, peaks)
            # attn_res_result = nar2_decoding(self, prefix_results=paraformer_greedy_result, encoder_outs=encoder_out,
            #                                       encoder_lens=torch.tensor([encoder_out.size(1)]), prefix_weight=attnre_weight, reverse_weight=0.0)
            # attn_res_token_int = attn_res_result[0].tokens
            # attn_res_token_int = list(filter(lambda x: x != self.eos and x != self.sos and x != self.blank_id, attn_res_token_int))
            ...
            # beam_size = kwargs.get("beam_size", 10)
            # paraformer_beam_result = paraformer_beam_search(decoder_out, ys_pad_lens, beam_size=beam_size, eos=self.eos)
            # attn_res_result = nar_ar_rescore(
            #     self,
            ...
            # CTC rescoring
            # beam_size = kwargs.get("beam_size", 10)
            # ctc_probs = self.ctc_logprobs(encoder_out)
            # ctc_beam_result = ctc_prefix_beam_search(
            #     ctc_probs=ctc_probs,
            #     ctc_lens=torch.tensor([encoder_out.size(1)]),
            #     beam_size=beam_size,
            #     context_graph=None,
            #     blank_id=0)
            # attn_res_result = attention_rescoring(
            #     self,
            #     ctc_prefix_results=ctc_beam_result,
            #     encoder_outs=encoder_out,
            #     encoder_lens=torch.tensor([encoder_out.size(1)]),
            #     ctc_weight=0.5,
            #     reverse_weight=0.0,)

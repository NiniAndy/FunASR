...
def nar2_decoding(
    ...
    for b in range(batch_size):
        ...
        hyps_lens = hyps_lens + 1  # Add <sos> at begining
        decoder_out, r_decoder_out = model.forward_attention_decoder(hyps_pad, hyps_lens, encoder_out, reverse_weight)
        nar2_output = decoder_out.argmax(dim=-1)[0]

        results.append(
            DecodeResult(tokens=nar2_output)
        )
    return results
...
def nar_ar_decoding(
    ...
    for b in range(batch_size):
        ...
        #     hyps_pad, _ = add_sos_eos(hyps_pad, sos, eos, model.ignore_id)
        #     hyps_lens = hyps_lens + 1  # Add <sos> at begining
        #     hyps_pad = hyps_pad[0, 1:].unsqueeze(0)
        #     eos = torch.tensor([2]).unsqueeze(1).to(hyps_pad.device)
        #     hyps_pad = torch.cat([hyps_pad, eos], dim=1)
        #     hyps_pad[0][0] = 0
        #
        #     for i in range(hyps_lens[0]):
        #         decoder_out, _ = model.forward_attention_decoder(hyps_pad, hyps_lens, encoder_out, reverse_weight)
        #         hyp_out = decoder_out.argmax(dim=-1)[0][i]
        #
        #         if i < hyps_lens[0]:
        #             if hyp_out != model.eos:
        #                 # update hyps_pad
        #                 hyps_pad[0][i] = hyp_out
        #                 hyps_pad[0][i + 1] = 0
        #             else:
        #                 break
        #         else:
...
def nar2_rescore(
    ...
    for b in range(batch_size):
        ...
        prefix_len = 1
        decoder_out, r_decoder_out = model.forward_attention_decoder(hyps_pad, hyps_lens, encoder_out, reverse_weight)
        nar2_output = decoder_out.argmax(dim=-1)
        # remove <sos>
        nar2_output = nar2_output[:, :-1]
        nar2_output = [tuple(sublist) for sublist in nar2_output.tolist()]
        # Only use decoder score for rescoring
        best_score = -float('inf')
        best_index = 0
        confidences = []
        ...
        for i, hyp in enumerate(nar2_output):
            now_prefix_tokens_confidence = prefix_tokens_confidence[i]
            score = 0.0
            tc = []  # tokens confidences
            for j, w in enumerate(hyp):
                s = decoder_out[i][j + (prefix_len - 1)][w]
                score += s
                tokens_confidences_num = math.exp(s)
                tokens_confidences_num = (1-prefix_weight)*tokens_confidences_num + prefix_weight*now_prefix_tokens_confidence[j]
                tc.append(tokens_confidences_num)
            score += decoder_out[i][len(hyp) + (prefix_len - 1)][eos]
            # add right to left decoder score
            if reverse_weight > 0 and r_decoder_out.dim() > 0:
        ...
        tokens_path_score = np.array(tokens_confidences)
        max_indices = np.argmax(tokens_path_score, axis=0)
        best_token_path = ()
        for a, z in enumerate(max_indices):
            best_token_path= best_token_path+(nar2_output[z][a],)

        # best_token_path = nar2_output[best_index]

...
from funasr.models.pretrain_asr.utils import load_cfg_from_dict
import os
...
@tables.register("model_classes", "PretrainASR_NAR")
class PretrainASRNAR(nn.Module):

    def __init__(
        self,
        pretrain_encoder: str = None,
        pretrain_encoder_conf: dict = None,
        decoder: str = None,
        ...
        **kwargs,
        ...
        pretrain_model_para = pretrain_model["model"]
        try:
            cfg = pretrain_model["cfg"]
            cfg_dict = cfg["model"]
        except KeyError:
            cfg_dict = pretrain_model["args"]
            cfg_dict = vars(cfg_dict)

        cfg = pretrain_cfg_classes()
        ...
        if predictor is not None:
            ...
            predictor_conf["idim"] = encoder_output_size
    ...
    def load_pretrained_model(self, model, pretrained_dict):
        model_dict = model.state_dict()

        # 检查和打印匹配的键和未匹配的键
        matched_keys = [k for k in pretrained_dict if k in model_dict]
        unmatched_keys = [k for k in pretrained_dict if k not in model_dict]
        ...
        return model, matched_keys, unmatched_keys, ignored_keys

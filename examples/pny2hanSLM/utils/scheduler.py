...
class CosineAnnealingWarmupRestarts(_LRScheduler):
    ...
    def get_lr(self):
        if self.step_in_cycle == -1:
            return self.base_lrs
        # 如果当前步骤在热身阶段内，则线性增加学习率
        elif self.step_in_cycle < self.warmup_steps:
            return [(self.max_lr - base_lr) * self.step_in_cycle / self.warmup_steps + base_lr for base_lr in self.base_lrs]
        # 如果当前步骤超过热身阶段，则使用余弦退火算法来调整学习率。
        else:
...
class FixedLR(_LRScheduler):
    ...
    This scheduler keeps the learning rate fixed to the initial lr set in the optimizer
    ...
    def get_lr(self):
        # Returns a list of the initial learning rates, as the lr remains fixed
        return self.base_lrs
...
class ExponentialLRScheduler(_LRScheduler):
    ...
    This scheduler updates the learning rate using an exponential decay based on the formula:
    lr = lr_base * (k_lr ** epoch)

    where lr_base is the initial learning rate and k_lr is a constant less than 1.
    """

    def __init__(
            self,
            optimizer: torch.optim.Optimizer,
            k_lr: Union[int, float],
            last_epoch: int = -1,
            **kwargs
    ):
        self.k_lr = k_lr
    ...
    def get_lr(self):
        """Returns the learning rate for the current epoch using an exponential decay."""
        if self.last_epoch == -1:
            # If last_epoch == -1, we are in the initial epoch, and the learning rate is unchanged.
            return self.base_lrs
        else:
            # Apply the exponential decay to the base learning rate
            return [base_lr * math.pow(self.k_lr, self.last_epoch) for base_lr in self.base_lrs]

    def __repr__(self):
        return f"{self.__class__.__name__}(k_lr={self.k_lr})"



if __name__ == '__main__':
    # Define a new simple model with a single parameter for demonstration purposes
    model = torch.nn.Linear(1, 1)

    # Reinitialize the optimizer with the new model's parameters
    optimizer = torch.optim.SGD(model.parameters(), lr=0.1)

    # fixed_lr_scheduler = FixedLR(optimizer, warmup_steps=2000)

    # # Reinitialize the WarmupLR scheduler with the new optimizer
    warmup_lr_scheduler = CosineAnnealingWarmupRestarts(optimizer,
                                                        first_cycle_steps=5000,
                                                          cycle_mult=1.0,
                                                          max_lr=0.1,
                                                          min_lr=0.001,
    ...
    # Simulate the learning rate schedule again
    lr_values = []
    for epoch in range(total_steps):
        warmup_lr_scheduler.step()
        lr_values.append(warmup_lr_scheduler.get_lr()[0])

    plt.figure(figsize=(10, 6))
    line, = plt.plot(range(1, total_steps + 1), lr_values, label='WarmupLR Schedule')
    plt.xlabel('Step')
    ...
    cursor.connect("add", lambda sel: sel.annotation.set_text(f"Step: {sel.target[0]:.0f}\nLR: {sel.target[1]:.6f}"))

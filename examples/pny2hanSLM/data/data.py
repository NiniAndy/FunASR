...
from examples.pny2hanSLM.data.dataset import ConfusionDataSet
from examples.pny2hanSLM.utils.tokenizer import init_tokenizer, init_pny_tokenizer
...
class MyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_dataset = ConfusionDataSet(self.tokenizer, self.train_path, self.confusion_conf, mode='train')
            self.valid_dataset = ConfusionDataSet(self.tokenizer, self.valid_path, self.confusion_conf, mode='valid')
            pass
        if stage == 'test' or stage is None:
            self.valid_dataset = ConfusionDataSet(self.tokenizer, self.valid_path, self.confusion_conf, mode='valid')
            self.test_dataset = ConfusionDataSet(self.tokenizer, self.test_path, self.confusion_conf, mode='test')
            pass
